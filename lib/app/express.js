...
module.exports = function(app, db) {
    ...
    var assets = require(appPath + '/assets/assets.json');
    assetmanager.init({
        js: assets.js,
        css: assets.css,
    ...
    app.use(favicon(appPath + '/assets/img/favicon.ico'));

    app.get(appPath + '/modules/aggregated.js', function(req, res) {
    ...
    function themeHandler(req, res) {
        ...
        gfs.files.findOne({
            ...
            if (err || !file) {
                fs.createReadStream(path.join(appPath, 'bower_components/bootstrap/dist/css/bootstrap.css')).pipe(res);
            } else {
                // streaming to gridfs
    ...
    app.get(appPath + '/bower_components/bootstrap/dist/css/bootstrap.css', themeHandler);

    app.get(appPath + '/modules/aggregated.css', function(req, res) {
        res.setHeader('content-type', 'text/css');
    ...
    mean.events.on('modulesFound', function() {
        ...
        for (var name in mean.modules) {
            app.use('/' + name, express.static(path.join(config.root, mean.modules[name].source, name, 'public')));
        }
        // Bootstrap models
        util.walk(appPath + '/server/models', null, function(model) {
            require(model);
        });
        ...
        // route by itself
        util.walk(appPath + '/server/routes', 'middlewares', function(route) {
            require(route)(app);
        });
