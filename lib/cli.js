...
exports.addKey = function() {
  ...
  var keys = shell.ls(home + '/.ssh/*.pub');
  ...
  prompt.get({
    ...
    function(err, result) {
      ...
      var sshKey = shell.cat(keys[result.key - 1]);
      ...
      publishPackage(pck, function(data) {
        ...
        shell.exec('git remote add meanio ' + data.repo, function(code) {
          if (!code) {
            ...
            console.log(chalk.green('Remote already exists'));
          }
        });
        ...
        shell.exec('git log -n 1', function(code) {
          ...
          if (!code) {
            ...
            shell.exec('git tag v' + data.version, function(code) {
              if (!code) {
                shell.exec('git push meanio v' + data.version, function(code) {
                  if (!code) {
                    console.log(chalk.green('Publish Success! ' + data.name + ' distribution ( master ) ' + data.version));
                  }
                });
              }
            });
            ...
            console.log(chalk.green('Use the `git push meanio master` to push your code'));
