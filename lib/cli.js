...
exports.pkg = function(name, options) {
    if (name === 'create') return createPkgRepo();
    else if (name === 'publish') return publishPkg();
    requiresRoot(function() {
        if (options.delete) {
...
var login = exports.login = function(email, password, callback) {
    var data = {
        name: email,
        password: password
    };
    var options = {
        uri: config.meanApi.host + '/meanApi/users/login',
        method: 'POST',
        form: data,
        headers: {
...
var createPkgRepo = exports.createPkgRepo = function() {
    var pkg;
    var user;
    console.log('---------- start requiresPkgRoot -------------------');
    requiresPkgRoot(function(data) {
        console.log(data.name);
        console.log('-------end requiresPkgRoot -------------------');
        pkg = data;
        ...
        prompt.get(schema, function(err, result) {
            login(result.name, result.password, function(info) {
                if (!info.err) {
                    user = info.data;
                    console.log(user);
                    console.log('---------end login ------------------');
                    console.log('---------start checKeys ------------------');
                    checKeys(user, function() {
                        console.log('---------end checKeys ------------------');
                        console.log('---------start existsPkg ------------------');
                        existsPkg(pkg.name, user.id, function(response) {
                            console.log(response);
                            console.log('---------end existsPkg ------------------');
                            if (response.err)
                                console.log(response.err);
                            else if (response.data) {
                                publishPkg();
                            } else {
                                console.log('---------start createNewPackage ------------------');
                                createNewPackage(pkg, user, function(test) {
                                    console.log('wow');
                                });
                            }
                        });
                    });
                } else console.log(info.err);
            });
        });
...
function requiresPkgRoot(callback) {
    loadPackageJson(process.cwd() + '/package.json', function(err, data) {
        if (err || data.name == 'mean') {
            console.log(chalk.yellow('Invalid MEAN app or not in or not in packages --package-- root'));
        } else {
            callback(data);
        }
    });
...
function checKeys(user, callback) {
    request(config.meanApi.host + '/meanApi/keys/?user=' + user.id + '&private_token=' + user.private_token, function(error, response, body) {
        console.log('checKeys');
        console.log(body);
        if (!error && response.statusCode == 200 && response.body.length) {
            console.log(response.body);
            callback(body);
        } else {
            console.log('Please add keys in meansite');
        }
    });
}

function existsPkg(name, uid, callback) {
    request(config.meanApi.host + '/meanApi/packages?name=' + name + '&user=' + uid, function(error, response, body) {
        if (error)
            console.log(error);
        else {
...
function createNewPackage(pkg, user, callback) {
    var options = {
        uri: config.meanApi.host + '/meanApi/packages/' + user.private_token,
        method: 'POST',
        form: pkg,
        headers: {
            'Content-Type': 'multipart/form-data'
    ...
    request(options, function(error, response, body) {
        if (!error && (response.statusCode === 200 || response.statusCode === 201)) {
            ...
            if (info.success) {
                ...
                console.log('---------start createRemoteBranch ------------------');
                createRemoteBranch(info.data, function() {
                    console.log('---------end createRemoteBranch ------------------');
                    callback(info);
                });
...
function createRemoteBranch(info, callback) {
    if (!shell.which('git')) return console.log(chalk.red('    Prerequisite not installed: git'));
    fs.writeFile('README.md', 'MEAN package', function(err) {
        if (err) throw err;
        console.log('It\'s saved!');
        var source = info.projectData.ssh_url_to_repo;
        progress.start();
        shell.exec('git init', function(code) {
            console.log('1 ' + code);
            progress.stop();
            if (code) return console.log(chalk.red('Error: git init failed'));
            shell.exec('git add README.md', function(code) {
                console.log('2 ' + code);
                if (!code) {
                    shell.exec('git commit -m "initial commit"', function(code) {
                        console.log('3 ' + code);
                        if (!code) {
...
var publishPkg = exports.publishPkg = function() {
    console.log('publishPkg');
    requiresPkgRoot(function(data) {
        existsPkg(data.name, null, function(response) {
            if (response.data) {
                if (!shell.which('git')) return console.log(chalk.red('    Prerequisite not installed: git'));
                progress.start();
                shell.exec('git add .', function(code) {
                    ...
                    console.log('2 ' + code);
                    if (!code) {
                        shell.exec('git commit -m "update project"', function(code) {
                            console.log('3 ' + code);
                            if (!code) {
                                shell.exec('git push origin master', function(code) {
                                    console.log('5 ' + code);
                                    if (!code) {
                                        console.log('finished');
                                    } else
                                        console.log('You do not have permissions to publish this package');
