...
var login = exports.login = function(email, password, callback) {
    ...
    request(options, function(error, response, body) {
        if (error && error.code === 'ECONNREFUSED') {
            console.log('problems with mean site');
        } else if (error) {
            console.log(error);
        } else {
                    ...
                    checKeys(user, function() {
                        existsPkg(pkg.name, user.id, function(response) {
                            if (response.err)
                                ...
                                console.log('mean package already exists');
                            } else {
                                createNewPackage(pkg, user, function(message) {
                                    console.log(message);
                                });
                            }
                        });
...
function requiresPkgRoot(callback) {
    loadPackageJson(process.cwd() + '/package.json', function(err, data) {
        if (err || data.name == 'mean') {
            console.log(chalk.yellow('Invalid MEAN app or not in or not in packages [package name] root'));
        } else {
            callback(data);
        }
    });
...
function createNewPackage(pkg, user, callback) {
    var options = {
        uri: config.meanApi.host + '/meanApi/package/create/' + user.private_token,
        method: 'POST',
        form: pkg,
        headers: {
            'Content-Type': 'multipart/form-data'
    ...
    request(options, function(error, response, body) {
        if (!error && (response.statusCode === 200 || response.statusCode === 201)) {
            var info = JSON.parse(body);
            if (info.success) {
                createRemoteBranch(info.data, function(message) {
                    callback(message);
                });
...
function createRemoteBranch(pkg, callback) {
    if (!shell.which('git')) return console.log(chalk.red('    Prerequisite not installed: git'));
    fs.writeFile('README.md', 'MEAN package', function(err) {
        if (err) throw err;
        console.log('It\'s saved!');
        var source = pkg.data.ssh_url_to_repo;
        progress.start();
...
function createTag(pkg, callback) {
    var options = {
        uri: config.meanApi.host + '/meanApi/package/tags/create',
        method: 'POST',
        form: pkg.data,
        headers: {
            'Content-Type': 'multipart/form-data'
