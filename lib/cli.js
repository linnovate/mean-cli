...
var MeanUsersApi = require('./mean-api/users');
                    ...
                    checKeys(user, function() {
                        ...
                        existsPkg(pkg.name, user.id, function(response) {
                            ...
                            if (response.success === false)
                                console.log(response.errDesc)
                            else if (response.success === true && response.exists === false) {
                                console.log('---------start createNewPackage ------------------');
                                createNewPackage(pkg, user, function(test) {
                                    console.log('wow');
                                }); //here we need to save package in mongo and create readme for initial commit
                            } else if (response.success === true && response.exists === true && response.authorized === true)
                                console.log('Please use mean publish');
                            else
                                console.log('Why ? ');
                        });
                    });
...
function existsPkg(name, uid, callback) {
    request(config.meanApi.host + '/meanApi/packages/' + name + '/user/' + uid, function(error, response, body) {
        if (error)
            console.log(error);
        else
            callback(body);
