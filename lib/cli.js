...
var login = exports.login = function(email, password, callback) {
    ...
    request(options, function(error, response, body) {
        if (error)
            ...
            if (callback)
                callback(info);
            // createRemoteBranch(info, function() {
    ...
    function createRemoteBranch(info, callback) {
        requiresPkgRoot(function(data) {
            var options = {
                uri: config.meanApi.host + '/meanApi/packages/' + info.data.private_token,
                method: 'POST',
                form: data,
                headers: {
                    'Content-Type': 'multipart/form-data'
            ...
            request(options, function(error, response, body) {
                if (!error && (response.statusCode === 200 || response.statusCode === 201)) {
                    ...
                    console.log(info);
                    if (!shell.which('git')) return console.log(chalk.red('    Prerequisite not installed: git'));
                    var source = info.data.projectData.ssh_url_to_repo;
                    progress.start();
...
var createPkgRepo = exports.createPkgRepo = function() {
    ...
    requiresPkgRoot(function(data) {
        pkg = data;
        console.log('Please login with mean login');
                    ...
                    checKeys(user, function() {
                        existsPkg(pkg.name, user.id, function(response) {
                            if (!response.success)
                                ...
                                createNewPackage(); //here we need to save package in mongo and create readme for initial commit
                            } else if (response.success && response.exists && response.authorized)
