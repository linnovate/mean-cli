  ...
  shell.exec('git clone -b ' + source, function(code) {
    ...
    loadPackageJson('./' + name + '/package.json', function(err, data) {
      ...
      fs.readFile(__dirname + '/../img/logo.txt', function(err, data) {
        ...
        console.log();
        shell.cd(name);
        if (options.pattern) pattern(options.pattern, options);
        if (options.quick) {
          npm.load(function(err, npm) {
            console.log('   installing dependencies...'.info);
            console.log();
            npm.commands.install(function(err) {
              ...
              console.log('   running the mean app...'.info);
              console.log();
...
function pattern(name, options) {
  var patternPath = __dirname + '/patterns/' + name;
  if (fs.existsSync(patternPath)) {
    shell.mkdir('-p', './packages');
    shell.cp(options.force ? '-Rf' : '-R', patternPath, './packages/');
    return console.log('   Pattern installed: '.info + name);
  } else {
    return console.log('   Pattern not available: '.warn + name);
  }
}
