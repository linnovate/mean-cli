...
exports.deploy = function() {
  ...
  loadPackageJson('./package.json', function(err, pck) {
    ...
    checKeys(function(keys) {
      ...
      console.log(chalk.green('    In doing so your app, you will creae a public remote and once pushed'));
      console.log(chalk.green('    will be available for all to see on the mean network and beyond'));
      console.log(chalk.green('    Pushing your code to master will deploy it in the cloud'));
      console.log();
      console.log(chalk.green('    I agree y/Y'));
      console.log(chalk.green('    No. Anything else'));
      prompt.start();
...
function getAppStatusTail(repo) {
  setTimeout(function() {
    appStatus(repo, true, 0);
  }, 1000);
}

function appStatus(repo, tail, status) {

  if (status === 3) return console.log(chalk.green('deployed to -> http://' + repo.replace('git@git.mean.io:', '').replace('.git', '').replace('/', '.') + '.cloud.mean.io'));
  ...
  request(options, function(error, response, body) {
    if (!error && (response.statusCode === 200 || response.statusCode === 201)) {
      body = JSON.parse(body);
      console.log(chalk.green('Status: ' + body.status) + ' message: ' + body.message);
      if (tail) return setTimeout(
        function() {
          if (body.status === 'finished') status++;
          appStatus(repo, tail, status);
        }, 1000);
    }
...
exports.init = function(options) {
  ...
  shell.exec('git clone -b ' + source, function(code, output) {
    ...
    loadPackageJson('./' + name + '/package.json', function(err, data) {
      ...
      loadPackageJson(taskdir + options.taskrunner + '.json', function(err, taskdeps) {
        if (err) return console.log(chalk.yellow('Could not read taskrunner deps file'));
