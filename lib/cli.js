...
exports.setPort = function(port) {
    requiresRoot(function() {
        fs.open('./server/config/env/all.js', 'rs', function(err, fd){
            if(err){
                console.log(err);
            }else{
                ...
                fs.read(fd,buffer, 0, buffer.length, null, function(err, bytesRead, buffer){
                    if(err){
                        ...
                        var newFile = buffer.toString().replace(/process.env.PORT \|\| \d+/gi, "process.env.PORT || "+port);
                        var newBuffer = new Buffer(bytesRead);
                        newBuffer.write(newFile);
                        //rewriting the new all.js 
                        fs.writeFile('./server/config/env/all.js', newBuffer, function(err){
                            if(err){
                                console.log(err);
                            }else{
