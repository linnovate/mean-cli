...
exports.toggleReporting = function(enable) {
	utils.loadPackageJson(utils.meanJsonPath(), function(err, settings) {
		if (err || !settings) console.log(err);
		settings.anonymizedData = enable; //true or false
		fs.writeFile(utils.meanJsonPath(), JSON.stringify(settings, null, 4), function(err) {
			if(err) console.log(err);
			else {
				var action = (enable) ? 'enable' : 'disable';
				console.log(action + ' reporting');
			}
		});
	});
};
