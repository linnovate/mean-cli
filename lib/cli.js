...
exports.install = function(module, options) {
  requiresRoot(function() {
    ...
    getPackage(packageName, function(data) {
      ...
      console.log('git clone --branch ' + tag + ' '+ cloneUrl + ' node_modules/' + data.name);
      
      var silentState = shell.config.silent; // save old silent state
      shell.config.silent = true;
      
      shell.exec('git clone --branch ' + tag + ' '+ cloneUrl + ' node_modules/' + data.name, function(code) {
        ...
        npm.load(function(err, npm) {
          ...
          npm.commands.install([pkgPath], function(err) {
            ...
            loadPackageJson('./node_modules/' + module[0] + '/package.json', function(err, data) {
