...
exports.updateMeanJson = function(values, callback) {

	var path = meanJsonPath();

	fs.lstat(path, function(err, stat) {
		if (err || !stat) {
			writeMeanJson(values, function() {
				callback();
			});
		} else {
			fs.readFile(path, function(err, file) {
				...
				writeMeanJson(json, function(err) {
					callback(err);
				});
					...
					writeMeanJson(data, function(err) {
						callback(err);
					});
				}
			});
...
function writeMeanJson(data, cb) {
	var path = meanJsonPath();
