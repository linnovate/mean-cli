...
Meanio.prototype.app = function(options, callback) {
	...
	var defaultConfig = require(options.config ? options.config + '/config': process.cwd() + '/server/config/config');
	...
	this.config = new Config(options.config, function(err, data) {
		...
		require(options.config ? options.config + '/passport' : process.cwd() + '/server/config/passport')(passport);
		...
		// Express settings
		var app = express();
		require(process.cwd() + '/server/config/express')(app, passport, database);
