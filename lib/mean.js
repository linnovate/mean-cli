...
function loadConfig () {
	...
	process.env.NODE_ENV = ~fs.readdirSync(process.cwd() + '/config').map(function(file) {
		return file.slice(0, -3);
	}).indexOf(process.env.NODE_ENV) ? process.env.NODE_ENV : 'development';

	return _.extend(
		require(process.cwd() + '/config/all'),
		require(process.cwd() + '/config/' + process.env.NODE_ENV) || {});
}
...
Meanio.prototype.app = function(options, callback) {
	...
	this.config = new Config(options.config, function(err, data) {
		...
		function bootstrapModels() {
			// Bootstrap models
			walk(process.cwd() + '/server/models', null, function(path) {
				require(path);
			});
		}
		...
		require(process.cwd() + '/server/passport')(passport);
		...
		// Express settings
		var app = express();
		require(process.cwd() + '/server/express')(app, passport, database);
