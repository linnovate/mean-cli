...
Meanio.prototype.app = function(name, options, callback) {
	...
	this.config = new this.Config(options.config, function(err, data) {
...
Meanio.prototype.Config = function(primary, callback) {

	//when doing a get we have no argunment and will return config option if it exists. if not we will call load
	//with an empty objecy

	///////if (arguments.length === 2) return loadConfig(arguments[0], arguments[1]);

	//////	if (arguments.length === 1 && typeof arguments[0] === 'function') return loadConfig({}, arguments[0]);

	//	if (arguments.length === 1 && typeof arguments[0] === 'object') return updateConfig(arguments[0]);

	if (config) return config.clean;

	load(primary, callback);
	...
	function update(settings, callback) {
		...
		Package.findOneAndUpdate({
			...
			load(settings, function(err, settings) {
	...
	function load(primary, callback) {

		var Package = loadPackageModel();

		if (!Package) return primary;

		Package.findOne({
			name: 'config'
		}, function(err, doc) {

			var original = JSON.flatten(primary, {
				hardCoded: true,
				default: true
			});

			var saved = JSON.flatten(doc ? doc.settings : primary, {});
	...
	function flatten(config, root) {
		for (var index in config) {
			if (config[index] && !config[index].value && typeof(config[index]) === 'object') {
				...
				if (options.hardCoded) {
					result[layerRoot(root, index)].hardCoded = true;
				}
			}
		}
	}
