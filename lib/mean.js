...
Meanio.prototype.app = function(name, options, callback) {
	...
	this.config(options.config, function(err, data) {
...
Meanio.prototype.config = function() {
	...
	if (!arguments.length) return config ? config.clean : {};
	...
	this.update = function(settings, callback) {
	...
	function loadConfig(primary, callback) {
		...
		Package.findOne({
			...
			var saved = JSON.flatten(doc ? doc.settings : {}, {});
