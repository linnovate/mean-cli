...
function aggregate(ext, aggPath, options) {
    ...
    var libs = true;
    if (aggPath){
        readFile(ext, path.join(process.cwd(), aggPath), libs, options, function(s){
    ...
    var processModules = function() {
        for (var name in modules) {
            ...
            readFiles(ext, modulePath, libs, options, whenDone);
        }
    };
...
function readFiles(ext, filepath, libs, options, callback) {
    fs.readdir(filepath, function(err, files) {
        ...
        var readNext = function(i) {
            ...
            var file = files[i];
            if (!libs && file !== 'assets') {
                readFile(ext, path.join(filepath, file), libs, options, function(s){
                    rtnString += s;
                    readNext(i+1);
                });
            } else {
...
function readFile(ext, filepath, libs, options, callback) {
    fs.readdir(filepath, function(err, files) {
        if (files){
            return readFiles(ext, filepath, libs, options, callback);
        }
