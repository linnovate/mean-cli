...
function updateSettings(options, done) {
	...
	var outputFilename = process.cwd() + '/' + options.name + '/settings.json';
...
function createFirstUser(options, done) {
	...
	var User, DB, email = options.mail;
...
function nextSteps(options) {
	...
	var questions = [{
		type: 'input,',
		name: 'register',
		message: 'Would you like to subscribe to network (Y/n) ? ',
		default: 'Y'
	}];
