...
function createLocalUser(options, done) {
	...
	console.log('Before you install the dependencies and fire up the server we can help you with setting up your first admin user.');
	var questions = [{
		type: 'input',
		name: 'createUser',
	...
	inquirer.prompt(questions, function(results) {
		if (results.createUser === 'Y' || results.createUser === 'y') userWizard(options, done);
		else done();
	});
}
...
function addGitRemote(name, done) {
  shell.exec('cd '+ name + ' && git remote rename origin upstream', function (status, error) {
    if (!status) {
      ...
      console.log('#############################################');
      console.log();
      console.log(chalk.green.bold('  Congratulations you have mean.io installed.'));
      console.log();
...
function createNetworkUser(options, done) {
	...
	if (options.anonymizedData) {
		...
		if (!token)
			registerOrLogin(options, done, function() {
				initApp(options.name, done);
			});
		else {
			initApp(options.name, done);
		}
