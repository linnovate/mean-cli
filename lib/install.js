...
function createFirstUser(options,done){
  // Conect to db
  // Check if user existst in db
  // Register to get pass? or as question?
  // if first time create in db as admin
  // Password used when created will be generated
  // If exists but not admin + make admin
  // In the end execute register if allready registered execute login
  // Make sure token is created and out put it

  console.log('Need to make some magic with:' + options.mail);
  done();
}
