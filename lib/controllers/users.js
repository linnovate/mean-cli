...
var shell = require('shelljs'),
	...
	anonymizedData = require('./anonymized-data'),
	napi = config.napiUrl,
	fs = require('fs');
...
var create = exports.create = function(options) {
	...
	var opt = {
		...
		headers: {
			'Content-Type': 'multipart/form-data',
			'Content-Length': querystring.stringify(options.user).length
		}
	...
	prompt.get({
			properties: {
				...
				username: {
					...
					required: true,
					defult: options.username ? options.username : ''
				},
				email: {
					format: 'email',
					required: true,
					defult: options.email ? options.email : ''
				},
				password: {
					minLength: 8,
					maxLength: 15,
