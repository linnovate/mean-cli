...
function checkPrivacy(options, callback) {
	...
	//var requireRoot = ['deploy', 'init', 'install', 'list', 'logs', 'package', 'postinstall', 'preinstall', 'publish', 'status', 'uninstall'];
	//var path = (options.name) ? process.cwd() + '/' + options.name + '/settings.json' : utils.meanJsonPath();
	utils.loadPackageJson('./settings.json', function(err, settings) {
		if (err || !settings) {
