...
function buildDirectoryStructure() {
  ...
  mkdir(path, function() {
    ...
    write(path + '/mean.json', readTemplate('mean.json'));
  });

  mkdir(path + '/server');

  mkdir(path + '/public');
  write(path + '/public/index.js', readTemplate('index.js'));
  ...
  mkdir(path + '/public/views', function() {
    ...
    write(path + '/public/views/example.html', readTemplate('client/example.html'));
  });
